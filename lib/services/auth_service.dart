...
class AuthService {
  ...
  // ESQUECEU A SENHA (Omitida para brevidade)
  ...
  // RESETAR SENHA (Omitida para brevidade)
  ...
  Future<Map<String, dynamic>> importWalletFromPrivateKey({
    ...
    final Map<String, dynamic> body = {
      ...
      "password": password,
      "force_replace": true, // Sempre true, como você pediu
    };
 ...
 // 5. Novo metodo para deletar a carteira
  Future<void> deleteWallet({required String password}) async {
   ...
   // 6. Novo metodo para transferir dinheiro(ETH)
  Future<Map<String, dynamic>> transferEth({
    required String password,
    required String toAddress,
    required double amount, // Usamos double para o valor
    ...
    // Usamos a sua função _securePost que já cuida do token e headers
    final response = await _securePost('wallet/transfer', body: body);

    // Retornamos a resposta completa para a UI poder acessar a "message"
    return response;
  }

  // 7. Novo método para buscar o historico de transações da wallet
  Future<List<Map<String, dynamic>>> getTransactions({int limit = 20}) async {
    // Montamos o endpoint com o parâmetro 'limit'
    final endpoint = 'wallet/transactions?limit=$limit';

    final responseData = await _secureGet(endpoint);

    // Acessamos a lista de transações dentro do objeto 'data'
    final data = responseData['data'];
    if (data != null && data['transactions'] is List) {
      // Retornamos a lista de transações
      return List<Map<String, dynamic>>.from(data['transactions']);
    } else {
      // Se não encontrar, retorna uma lista vazia para evitar erros
      return [];
    }
  }
